...
def rewrite_telegram_url(
    ...
    match = TELEGRAM_LINK_PATTERN.fullmatch(url)
    if not match:
        return url
    ...
    mapped = get_mapping(source_chat, 0, message_id)

    if not mapped:
        return url

    target_chat, target_topic, target_msg = mapped
    target_chat_clean = str(target_chat).replace("-100", "")

    if str(target_chat).lstrip("-").isdigit():
        return f"https://t.me/c/{target_chat_clean}/{target_msg}"

    return f"https://t.me/{target_chat_clean}/{target_msg}"
...
def entity_to_bot_entity(entity):
    ...
    kwargs = {
        "type": mapping.get(entity_type, "text_mention"),
...
async def send_text_message(message, target_chat, target_topic, config):
    ...
    processed = replace_plain_text(text, config)
    kwargs = {}
    ...
    if message.entities:
        kwargs["entities"] = [
            entity_to_bot_entity(e) for e in message.entities
        ]
...
def process_caption(caption, config):
    if not caption:
        return None
    ...
    return config["prefix"] + caption + config["suffix"]


async def send_media_message(message, target_chat, target_topic, config):
    caption = process_caption(message.caption, config)
    kwargs = {}
...
async def clone_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status = await update.effective_message.reply_text("🚀 Transfer started...")
    ...
    for index, message in enumerate(messages, start=1):
        try:
            result = None
            if message.text:
                result = await send_text_message(message, target, tgt_topic, config)
            else:
                result = await send_media_message(message, target, tgt_topic, config)
        ...
        if index == 1 or index % 5 == 0 or index == len(messages):
            percentage = (index / len(messages) * 100) if messages else 100
            try:
...
async def set_replace(update, context):
    if not context.args or " | " not in update.effective_message.text:
        await update.effective_message.reply_text("Usage:\n/setreplace old | new")
        return
    value = update.effective_message.text.split(maxsplit=1)[1]
...
async def set_link(update, context):
    ...
    await update.effective_message.reply_text("✅ Link mapping configured.")
...
async def setup_commands(application):
    await application.bot.set_my_commands([
        ...
        BotCommand("setlink", "Configure links"),
...
def main():
    ...
    async def run():
        ...
        # Keep running via event loop stop condition
